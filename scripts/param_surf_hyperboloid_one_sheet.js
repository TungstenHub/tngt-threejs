...
const {THREE, renderer, scene, camera, gui} = init('param_surf_hyperboloid_one_sheet', {
  gui: true,
  orbitControls: true,
  cameraPos: [0,0,5]
});
...
const surfaceParamGenerator = function (data) {
	...
	return function ( u, v, optionalTarget ) {
		...
		const x = Math.cos(u) * v*Math.cos(data.a) - Math.sin(u);
		const z = Math.sin(u) * v*Math.cos(data.a) + Math.cos(u);
		const y = v*Math.sin(data.a);
...
gui.add( surfaceData, 'a', 0, Math.PI/2 ).onChange( updateSurface );
